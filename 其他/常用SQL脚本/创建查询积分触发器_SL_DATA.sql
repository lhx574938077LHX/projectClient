--创建SLDATA_INTEGRAL_TRIGGER触发器
DECLARE t_num NUMBER;
BEGIN
  SELECT COUNT(1) INTO t_num FROM user_triggers WHERE trigger_name='SLDATA_INTEGRAL_TRIGGER';
  IF t_num = 1 THEN
    EXECUTE IMMEDIATE 'DROP TRIGGER SLDATA_INTEGRAL_TRIGGER';
  END IF;
END;
/
CREATE TRIGGER SLDATA_INTEGRAL_TRIGGER
BEFORE INSERT ON SL_DATA FOR EACH ROW
DECLARE
...
BEGIN 
  ...
  */
  ...
  UPDATE COMPANY_INFO SET INTEGRAL=INTEGRAL+V_INTEGRAL_SUM WHERE ID = :NEW.COMPANY_ID;
   INSERT INTO INTEGRAL_LOG(ID,REQ_COMPANY_ID,REQ_COMPANY_CODE,RSP_COMPANY_ID,RSP_COMPANY_CODE,SEARCH_LOG_ID,SEARCH_TRX_NO,BORROW_TYPE,BORROW_STATE,BORROW_AMOUNT,CONTRACT_DATE,LOAN_PERIOD,REPAY_STATE,ARREARS_AMOUNT,INTEGRAL_SUM,REC_TIME)
    VALUES (INTEGRAL_LOG_ID_SEQUENCE.NEXTVAL,/*REQ_COMPANY_ID*/0,/*REQ_COMPANY_CODE*/'',:NEW.COMPANY_ID,:NEW.COMPANY_CODE,/*:NEW.SEARCH_LOG_ID*/0,/*:NEW.SEARCH_TRX_NO*/'',V_BORROW_TYPE,V_BORROW_STATE,V_BORROW_AMOUNT,V_CONTRACT_DATE,V_LOAN_PERIOD,V_REPAY_STATE,V_ARREARS_AMOUNT,V_INTEGRAL_SUM,:NEW.REC_TIME); 
END;